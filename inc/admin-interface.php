 ...
 * Name icons cpt-[POST-TYPE].png and place into img/icons/ (in child theme)
 *
 * @since	Pilau_Base 0.1
 * @todo	Document online
 */
if ( ! function_exists( 'pilau_cpt_icons' ) ) {
	...
	function pilau_cpt_icons() {
		...
		if ( $cpts ) {
			...
			<style media="screen">
				<?php
				foreach ( $cpts as $cpt ) {
					if ( file_exists( get_stylesheet_directory() . '/img/icons/cpt-' . $cpt . '.png' ) ) {
						?>
						#menu-posts-<?php echo $cpt; ?> .wp-menu-image { background: url('<?php echo get_stylesheet_directory_uri(); ?>/img/icons/cpt-<?php echo $cpt; ?>.png') no-repeat 6px -17px !important; }
